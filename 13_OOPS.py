...
class OurBank:
    ...
    def increment_cust_count(cls):
        cls.Total_no_of_customers = cls.Total_no_of_customers + 1
        print("Time is -> ",OurBank.get_ts())
        print("Total no of customers registered in the bank is -> ", cls.Total_no_of_customers)

    # Method inside a class which does not depend on either instance variables or Class variables
    # It can work with either instance or class level variables to validate or conditional checking
    # Utility function
    @staticmethod
    def get_ts():
        return datetime.now()
